...
| `CLIPPYME_LAUGHTER_DETECTION`                                  | `0`                                 | Local laughter/audio-event detection (sherpa-onnx) for Deepgram/Whisper transcripts — backfills the `(laughter)` signal ElevenLabs gives natively, at no cloud cost.                                                                                                              |
...
| `CLIPPYME_MIN_CLIP_DURATION`                                   | `75`                                | Lower bound the Gemini prompt is told it can pick. Prompt-enforced only — the sentence-snap stage has no min-duration check of its own.                                                                                                                                            |
| `CLIPPYME_MAX_CLIP_DURATION`                                   | `180`                               | Upper bound the Gemini prompt is told it can pick AND the sentence-snap extension ceiling — the two always read the same value, so raising it (e.g. `90`/`120` for storytelling content) never leaves them disagreeing.                                                          |
