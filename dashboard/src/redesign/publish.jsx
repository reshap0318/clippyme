...
const DEFAULT_CAPTION_TEMPLATE = "{caption}\n\n{hashtagai}";
...
export function PublishModal({
  ...
  // {caption} = the clip's own caption (falls back to its title),
  // {title} = the clip's title, {hashtagai} = its Gemini hashtags.
  ...
  const buildBody = (clip, idx, batchPos = 0) => {
    ...
    const finalCaption = dedupeHashtagsInText(renderCaptionTemplate(template, {
      caption: clip.tiktok_caption || title,
      title,
      hashtagai: combineHashtags(clip.hashtags).join(" "),
    })) || title;
    return {
      title,
