...
test('forceReframe: same mode still calls reframeClip (Force re-render toggle)', async () => {
  const { calls, args } = makeCtx();
  await runApplyEdit({ ...args, params: baseParams({ forceReframe: true }) });
  expect(args.api.reframeClip).toHaveBeenCalledWith(JOB, 2, 'auto', undefined, undefined);
...
test('reframe-only: calls reframeClip, busts cache, never composes', async () => {
  ...
  expect(args.api.reframeClip).toHaveBeenCalledWith(JOB, 2, 'subject', undefined, undefined);
  expect(args.api.composeClip).not.toHaveBeenCalled();
  const bust = calls.states.find((p) => p.reframeBust);
  expect(bust).toMatchObject({ reframeBust: 1234, previewUrl: undefined });
...
test('reframe-only: passes letterboxFill through to reframeClip', async () => {
  const { args } = makeCtx();
  await runApplyEdit({
    ...args,
    params: baseParams({ reframeMode: 'disabled', letterboxFill: 'blur' }),
  });
  expect(args.api.reframeClip).toHaveBeenCalledWith(JOB, 2, 'disabled', undefined, 'blur');
});
...
test('apiIdx resolves the backend call independently of the local idx (I-1 gap)', async () => {
  ...
  expect(args.api.reframeClip).toHaveBeenCalledWith(JOB, 5, 'subject', undefined, undefined);
