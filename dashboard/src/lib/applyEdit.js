...
export async function runApplyEdit({ jobId, idx, apiIdx = idx, params, api, updateClipState, pushToast, now = Date.now }) {
  const { reframeMode, baseMode, letterboxZoom, letterboxFill, toggles, subtitleParams, hookParams, logoParams, gradeParams, bannerParams, dropRanges, forceReframe } = params;
  ...
  updateClipState(idx, { reframeMode, letterboxZoom, letterboxFill, toggles, subtitleParams, hookParams, logoParams, gradeParams, bannerParams, dropRanges,
  ...
  try {
    if (reframeChanged) {
      await api.reframeClip(jobId, apiIdx, reframeMode, letterboxZoom, letterboxFill);
      reframeApplied = true;
